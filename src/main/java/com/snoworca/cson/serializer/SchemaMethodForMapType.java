...
class SchemaMethodForMapType extends SchemaMethod implements ISchemaMapValue {
    ...
    SchemaMethodForMapType(TypeElement parentsTypeElement, Method method) {
        ...
        if(!String.class.isAssignableFrom(keyClass)) {
            if(isGetter) {
                throw new CSONObjectException("The key of Map, which is the return value of the method, is not of String type. Please use String key. (path: " + methodPath + ")");
            } else {
                throw new CSONObjectException("The key of Map, which is the parameter of the method, is not of String type. Please use String key. (path: " + methodPath + ")");
            }
        }
    ...
    public Object newInstance() {
        try {
            ...
            throw new CSONObjectException("Map type " + getValueTypeClass().getName() + " has no default constructor. (path: " + methodPath + ")", e);
        }
    }
}
